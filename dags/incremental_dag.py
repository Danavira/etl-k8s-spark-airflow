...
SPARK_DRIVER_CORES = os.getenv("SPARK_DRIVER_CORES")
SPARK_DRIVER_MEMORY = os.getenv("SPARK_DRIVER_MEMORY")
SPARK_DRIVER_MEMORY_OVERHEAD = os.getenv("SPARK_DRIVER_MEMORY_OVERHEAD")
...
SPARK_EXECUTOR_MEMORY_OVERHEAD = os.getenv("SPARK_EXECUTOR_MEMORY_OVERHEAD")
...
with DAG(
    dag_id="run_incremental",
    schedule="0 0 * * *",                  # DAG runs once a day at midnight.
    start_date=pendulum.datetime(2025, 8, 13, 15, 0, tz=tz),  # start date has to be static and in the past.
    catchup=False,                           # only fire the next tick
    is_paused_upon_creation=False,
    default_args=default_args,
) as dag:

   spark_job = KubernetesPodOperator(
        task_id="etl_incremental_job",
        name="etl-incremental",                         # name of the pod that will appear in kubectl get pods
        namespace=NAMESPACE,                         
        image=SPARK_IMAGE, 
