...
with DAG(
    dag_id="run_backfill",
    schedule="0 4,8,12,16,20 * * *",                  # DAG runs every 4 hours, excluding midnight, to make room for ETL.
    start_date=pendulum.datetime(2025, 9, 8, 15, 0, tz=tz),  # start date has to be static and in the past.
    ...
    default_args=default_args
     ...
     spark_job = KubernetesPodOperator(
        ...
        name="etl-backfill",                         # name of the pod that will appear in kubectl get pods
        namespace=NAMESPACE,                         
        image=SPARK_IMAGE, 
